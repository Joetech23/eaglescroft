...
import { featuredProjects, projects, shotTall } from '@/lib/projects'
...
export default function LivePortfolio() {
  ...
  return (
    <section id="work" className="section-dark relative overflow-hidden px-6 py-[100px] sm:px-7 sm:py-[120px]">
      <div className="bg-dots-dark pointer-events-none absolute inset-0 opacity-60" />
      <div className="pointer-events-none absolute left-1/2 top-[-10%] h-[500px] w-[900px] max-w-full -translate-x-1/2"
        style={{ background: 'radial-gradient(60% 60% at 50% 0%,rgba(37,99,235,.28),transparent 70%)' }} />

      <div className="relative mx-auto max-w-[1280px]">
        <div className="mb-10 flex flex-wrap items-end justify-between gap-6">
          <div>
            <span className="chip-dark">Selected work</span>
            ...
            {shown.map((p) => {
              ...
              return (
                <motion.a
                  ...
                  className="group block overflow-hidden rounded-[22px] border border-white/10 bg-[#081234] no-underline shadow-dark-card transition-all duration-300 hover:-translate-y-2 hover:border-gold-400/50 hover:shadow-[0_1px_0_rgba(255,255,255,0.06)_inset,0_30px_70px_-12px_rgba(0,0,0,0.7),0_0_40px_-8px_rgba(37,99,235,0.35)]"
                >
                  {/* browser chrome */}
                  <div className="flex items-center gap-3 border-b border-white/[0.07] bg-[#0b1740] px-4 py-3">
                    <div className="flex gap-[7px]">
                      <span className="h-[11px] w-[11px] rounded-full bg-[#ff5f57]" />
                      <span className="h-[11px] w-[11px] rounded-full bg-[#febc2e]" />
                      <span className="h-[11px] w-[11px] rounded-full bg-[#28c840]" />
                    </div>
                    <div className="flex flex-1 items-center gap-2 overflow-hidden rounded-[9px] bg-black/30 px-3 py-1.5 font-mono text-[11.5px] text-white/60">
                  ...
                  <div className="px-5 pb-[22px] pt-5">
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <ArrowUpRight className="h-[18px] w-[18px] flex-none text-gold-300" />
                    </div>
                    <div className="my-[5px] mb-[13px] font-mono text-[11px] uppercase tracking-[0.08em] text-gold-300/90">{p.industry}</div>
                    <p className="mb-[15px] text-[13.5px] leading-snug text-white/60">{p.description}</p>
                    <div className="flex flex-wrap gap-1.5">
                      {p.stack.slice(0, 3).map((s) => (
                        <span key={s} className="rounded-lg bg-white/[0.07] px-[9px] py-1 font-mono text-[10.5px] text-white/55">{s}</span>
                      ))}
                    </div>
                  </div>
                </motion.a>
        ...
        <div className="mt-12 flex justify-center">
          <a
            href="/portfolio"
            className="group inline-flex items-center gap-2.5 rounded-full bg-white px-7 py-4 text-[15px] font-semibold text-navy shadow-[inset_0_1px_0_rgba(255,255,255,0.8),0_10px_30px_-6px_rgba(0,0,0,0.5)] transition-all duration-300 hover:-translate-y-0.5 hover:shadow-[inset_0_1px_0_rgba(255,255,255,0.8),0_16px_40px_-6px_rgba(37,99,235,0.5)]"
          >
            View all work
